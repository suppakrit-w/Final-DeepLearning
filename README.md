...
﻿**การจำแนกประเภทตุ่มเเละรอยโรคผิวหนัง 7 ชนิด จากชุดข้อมูล HAM10000 \
โดยใช้ Transfer Learning และ Weighted Sampling**

1. **ความสำคัญและความน่าสนใจ**
...
การวินิจฉัยที่ล่าช้าโดยเฉพาะกับรอยโรคชนิดเมลาโนมา (Melanoma) สามารถส่งผลกระทบโดยตรงต่ออัตราการรอดชีวิตของผู้ป่วย (Esteva, et al. 2017, 115-118) ด้วยเหตุนี้สถาปัตยกรรม Deep Learning ประเภท Convolutional Neural Networks (CNNs) จึงได้เข้ามามีบทบาทสำคัญในฐานะเครื่องมือช่วยสนับสนุนการตัดสินใจทางการแพทย์โดยมีความสามารถในการวิเคราะห์และจำแนกคุณลักษณะที่ซับซ้อนจากภาพถ่ายทางการแพทย์ได้อย่างแม่นยำ (Esteva, et al. 2017, 115-118)

อีกทั้งปัญหาของโปรเจกต์นี้ไม่ได้อยู่ที่การจำแนกภาพเพียงอย่างเดียว แต่แฝงอยู่ในลักษณะโครงสร้างของชุดข้อมูล HAM10000 ซึ่งเป็นปัญหาที่เกิดขึ้นจริงในข้อมูลทางการแพทย์ นั่นคือปัญหาข้อมูลไม่สมดุล (Tschandl, et al. 2018) โดยชุดข้อมูลนี้ประกอบด้วยคลาสส่วนใหญ่ เช่น ไฝธรรมดา (nv) ที่มีจำนวนข้อมูลหลายพันภาพ ในขณะที่คลาสส่วนน้อย ซึ่งเป็นคลาสที่มีความสำคัญทางการแพทย์สูงสุด เช่น มะเร็งเมลาโนมา (mel) หรือ มะเร็งเบซัลเซลล์ (bcc) กลับมีจำนวนข้อมูลเพียงหลักร้อยเท่านั้น (Tschandl, et al. 2018) ซึ่งความสำคัญในการแก้ไขปัญหานี้อยู่ที่การป้องกันไม่ให้โมเดลเกิด Bias หากโมเดลถูกฝึกแบบปกติ มันทายแต่คลาส nv เพื่อให้ได้ค่าความแม่นยำโดยรวมที่สูง ซึ่งจะส่งผลให้โมเดลไร้ประสิทธิภาพในการตรวจจับรอยโรคที่อันตรายและหายาก ซึ่งถือเป็นเป้าหมายหลักทางการแพทย์ที่แท้จริง
...
2. **เหตุผลที่จำเป็นต้องใช้ deep learning ในการแก้ปัญหา**
...
`	`ซึ่งวิธีอื่นๆ ที่เราไม่ได้เลือกใช้ เช่น Support Vector Machines, K-Nearest Neighbors, หรือ Random Forest มีข้อจำกัดที่สำคัญที่สุดคือ Manual Feature Extraction ถ้าหากเราจะใช้ SVM ในการจำแนกภาพรอยโรค เราไม่สามารถป้อน Raw Pixels เข้าไปในโมเดลได้โดยตรง เราจำเป็นต้องให้ผู้เชี่ยวชาญ มาออกแบบและสกัด Features ของภาพออกมาก่อน
   ...
   2.2. **จุดเด่นของ Deep Learning (CNNs) ที่ใช้ในโปรเจกต์**
**\
`	`Automatic Feature Extraction เป็นจุดเด่นที่สำคัญที่สุด และเป็นเหตุผลหลักที่ต้องใช้โมเดล CNN เช่น ResNet, EfficientNet ที่โมเดลจะเรียนรู้คุณลักษณะเหล่านี้ได้เองโดยอัตโนมัติจากพิกเซลใน Layer แรก ๆ โดยจะเรียนรู้สิ่งง่ายๆ เช่น เส้นขอบ, จุดสี และชั้นที่ลึกขึ้นจะนำสิ่งเหล่านั้นมาประกอบกันเป็นคุณลักษณะที่ซับซ้อน เช่น พื้นผิวแบบตาข่ายที่พบบ่อยใน mel หรือ โครงสร้างแบบกิ่งไม้ที่พบบ่อยใน bcc (Esteva, et al. 2017, 115-118) โดยวิธีคือ เรานำโมเดล ResNet50 หรือ EfficientNet-B0 ที่ฉลาดอยู่แล้วมาใช้ จากนั้นเราทำการ Freeze ความรู้นั้นไว้ และสร้าง New Head เพื่อสอนให้มันรู้จักโรคผิวหนัง 7 ชนิดของเราเท่านั้น
   ...
   2.3. **จุดด้อยของ Deep Learning (CNNs) ที่ใช้ในโปรเจกต์**
   1. โมเดล Deep Learning เช่น ResNet มีความซับซ้อนสูงมาก ทำให้ยากต่อการ Interpretability ว่าทำไมมันถึงตัดสินใจว่าภาพนี้เป็น mel มันอาจจะมองเห็นคุณลักษณะที่เราไม่รู้จัก
   1. ถ้าหากเราไม่ใช้ Transfer Learning เช่นโมเดล Custom CNN ที่สร้างเอง โมเดลของเรามักจะมีประสิทธิภาพต่ำมาก	



1. **สถาปัตยกรรม deep learning ที่ใช้**
...
โมเดลนี้ใช้เทคนิค Transfer Learning โดยนำสถาปัตยกรรม ResNet50 ซึ่งเป็นโมเดลที่ได้รับการพิสูจน์แล้วว่ามีประสิทธิภาพสูงในการจำแนกภาพจากชุดข้อมูล ImageNet (He, et al. 2016, 770-778) ResNet \
มีจุดเด่นคือการใช้ Residual Connections หรือ Shortcut Connections ที่ช่วยให้โมเดลสามารถเรียนรู้ในชั้น\
ที่ลึกมากได้โดยไม่เกิดปัญหา Gradient Degradation (He, et al. 2016, 770-778)  ในการทดลองนี้ \
เราใช้ ResNet50 ในรูปแบบดังนี้

1. Frozen โดยโหลดโมเดล ResNet50 ที่ Pre-train มาแล้ว และทำการ Freeze พารามิเตอร์ Weights และ Biases ทั้งหมดในส่วนนี้ เพื่อไม่ให้มีการอัปเดตระหว่างการฝึกใหม่
1. Trainable โดยเราตัดส่วน Classifier เดิมของ ResNet50 ทิ้งไป และสร้าง Head ใหม่ของเราเองขึ้นมาแทนที่ในชั้น Fully Connected โดยมีสถาปัตยกรรมดังนี้
   1. ` `nn.Linear(in\_features=2048, out\_features=512) คือทำการรับ 2048 Nodesจากชั้น Global Average Pooling ของ ResNet50
   1. ` `nn.ReLU() ใช้ ReLU เป็น Activation Function
...
1. Frozen โดยโหลดโมเดล EfficientNet-B0 ที่ Pre-train มาแล้ว และทำการ Freeze พารามิเตอร์ ทั้งหมดในส่วนนี้
...
AI-generated content may be incorrect.](Aspose.Words.6a844693-d34b-4eaa-a347-df9c340031b9.005.png)
...
`	`- running\_loss += loss.item() \* inputs.size(0) คำนวน Loss รวมของ batch เพื่อนำไปทำกราฟ\
`	`- correct\_train += (predicted == labels).sum().item() นับจำนวนค่าที่ทายถูก\
...
1. **อธิบายวิธีในการ train และ Dataset ที่ใช้**
...
1. ` `โรคผิวหนังจากแสงแดด(Actinic keratoses - akiec)
1. ` `มะเร็งเบซัลเซลล์ (Basal cell carcinoma - bcc)
1. ` `ตุ่มคล้ายเนื้องอกเคอราโตซิส (Benign keratosis-like lesions - bkl)
1. ` `เนื้องอกผิวหนัง (Dermatofibroma - df)
1. ` `มะเร็งเมลาโนมา (Melanoma - mel)
1. ` `ไฝ (Melanocytic - nv) 
...
1. ` `Data Augmentation โดยข้อมูลใน Training Set ถูกเพิ่มปริมาณโดยใช้ transforms \
   ` `ชุดเดียวกัน คือ
...
- ทำการแปลงภาพเป็น Tensor และ Normalize ค่าสีด้วย Mean/Std ของ ImageNet
  1. Imbalance Handling โดยโมเดลทั้งสามใช้ WeightedRandomSampler ใน DataLoader ของ Training Set เพื่อทำโมเดลเห็นคลาสที่หายาก เช่น mel, df บ่อยขึ้น และเห็นคลาส nv น้อยลง
  1. ` `Loss Function โดย nn.CrossEntropyLoss ถูกใช้เป็นฟังก์ชันสูญเสียสำหรับทุกโมเดล

  1. ` `Adam Optimizer (lr=0.001) ถูกใช้เป็นตัวปรับค่า Weight
  1. ` `Training Epochs ในทุกโมเดลถูกฝึกเป็นเวลา 15 Epochs เท่ากัน

ในการทดลองนี้มีสถาปัตยกรรมโมเดลเป็นตัว Independent Variable เพื่อวัดผลลัพธ์ เช่น Accuracy และ Recall ที่เกิดขึ้น

1. **อธิบายการประเมิน (evaluate) model**
...
1. **Metric ที่เหมาะสมในการประเมิน**
...
1. ![A graph of different colored lines

AI-generated content may be incorrect.](Aspose.Words.6a844693-d34b-4eaa-a347-df9c340031b9.007.png) Accuracy และ Loss ของโมเดล Custom**\
...
1. **Model Classification Results and Confusion Matrix Analysis**
   1. ![A graph with numbers and a number in a row

AI-generated content may be incorrect.](Aspose.Words.6a844693-d34b-4eaa-a347-df9c340031b9.010.png)Confusion Matrix ของโมเดล Custom
...
1. ![A graph with numbers and a number in the middle

AI-generated content may be incorrect.](Aspose.Words.6a844693-d34b-4eaa-a347-df9c340031b9.011.png)Confusion Matrix ของโมเดล ResNet50
...
AI-generated content may be incorrect.](Aspose.Words.6a844693-d34b-4eaa-a347-df9c340031b9.012.png)**C**onfusion Matrix ของโมเดล EfficientNet-B0
...
1. **ผลลัพธ์การประเมินโมเดล (Model Evaluation Results)**

**ตารางที่ 1** ผลการประเมินสมรรถนะของโมเดลจำแนกมะเร็งผิวหนัง

|**Metric**|**Custom CNN (Baseline)**|**ResNet50**|**EfficientNet-B0 <br>(ProposedWinner)**|
| :- | :-: | :-: | :-: |
...
1. **บทความอ้างอิงและงานที่เกี่ยวข้อง**
...
1. **สมาชิกผู้จัดทำ และสัดส่วนและการแบ่งหน้าที่ความรับผิดชอบ**
